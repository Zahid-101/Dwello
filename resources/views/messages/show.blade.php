        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden flex flex-col"
            ...
            <div class="p-4 border-b bg-white flex items-center justify-between shadow-sm z-10">
                <div class="flex items-center gap-3">
                    ...
                    <div
                        class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-lg shadow">
                ...
                @if($conversation->status !== 'rejected')
                    <form action="{{ route('conversations.reject', $conversation) }}" method="POST" onsubmit="return confirm('Are you sure you want to block this user? You will not be able to message each other.');">
                        @csrf
                        <button type="submit" class="text-red-500 hover:text-red-700 font-medium text-sm flex items-center gap-1 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-full transition">
                ...
                @elseif($conversation->status === 'rejected' && $conversation->blocked_by == auth()->id())
                     <form action="{{ route('conversations.unblock', $conversation) }}" method="POST">
                        @csrf
                        <button type="submit" class="px-3 py-1.5 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium text-xs transition border border-gray-300">
                            Unblock User
                        </button>
                    </form>
                @endif
                ...
                @foreach($messages as $message)
                    ...
                    <div class="message-wrapper {{ $isMine ? 'items-end' : 'items-start' }}" data-id="{{ $message->id }}">
                        @if($message->property_id && $message->property)
                            <!-- Rich Property Card -->
                            <div class="message-bubble {{ $isMine ? 'mine' : 'theirs' }}" style="padding: 0; overflow: hidden; max-width: 300px; background: white; color: black; border: 1px solid #e5e7eb;">
                                @php
                                ...
                                <div style="padding: 12px;">
                                    <h4 style="font-weight: 600; font-size: 15px; margin-bottom: 4px; line-height: 1.3;">{{ Str::limit($message->property->title, 40) }}</h4>
                                    <div style="color: #f97316; font-weight: 700; margin-bottom: 8px;">
                                        LKR {{ number_format($message->property->monthly_rent/1000, 1) }}k <span style="font-weight: 400; color: #6b7280; font-size: 12px;">/mo</span>
                                    </div>
                                    <a href="{{ route('properties.show', $message->property) }}" class="block w-full text-center bg-gray-900 text-white rounded-lg py-2 text-sm font-medium hover:bg-gray-800 transition">
                                        View Property
                                    </a>
                                </div>
                            </div>
